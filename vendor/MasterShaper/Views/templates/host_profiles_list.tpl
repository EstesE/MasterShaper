...
<div class="ui grid">
 <div class="ten wide column">
  <div class="left aligned container">
   <h1 class="ui header"><i class="archive icon"></i>Host Profiles</h1>
  </div>
 </div>
...
{host_profiles_list}
 <tr>
  ...
  <td>
   <div name="host_profile_{$item->getId()}" class="filterable inline editable content" data-current-value="{$item->getName()}" data-orig-value="{$item->getName()}" style="float: left;">{$item->getName()}</div>&nbsp;
   <a name="host_profile_{$item->getId()}" class="inline editable edit link" data-inline-name="host_profile_{$item->getId()}"><i class="tiny edit icon"></i></a>
   <div name="host_profile_{$item->getId()}" class="inline editable formsrc" style="display: none;">
    <form class="ui form" onsubmit="return false;">
     <div class="fields">
      <div class="field small ui input">
       <input type="text" name="host_profile_{$item->getId()}" value="{$item->getName()}" data-action="update" data-model="host_profile" data-key="host_name" data-id="{$item->getId()}" />
      </div>
      <div class="field">
  ...
  <td>
   <div class="ui icon buttons">
    <a id="edit_link_{$item->getId()}" href="{get_url page='host-profiles' mode='edit' id=$item->getSafeLink()}" class="edit item ui icon button action link"><i class="edit icon"></i></a>
    <a id="delete_link_{$item->getId()}" class="delete item ui icon button action link" data-action-title="Deleting {$item->getName()|escape}" data-modal-title="Delete {$item->getName()|escape}" data-modal-text="Please confirm to delete {$item->getName()|escape}" data-id="{$item->getId()}" data-guid="{$item->getGuid()}" data-model="host_profile" data-content="Delete {$item->getName()|escape}" data-variation="wide"><i class="remove circle icon"></i></a>

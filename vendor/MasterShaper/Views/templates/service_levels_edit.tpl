...
{if !isset($sl->sl_idx) || empty($sl->sl_idx)}
...
<table style="width: 100%;" class="withborder2">
 ...
 <tr>
  <td style="text-align: center;"><a href="{get_url page='Service Levels List'}" title="Back"><img src="{$icon_arrow_left}" alt="arrow left icon" /></a></td>
...
{foreach from=$obj_used key=obj_idx item=obj name=objects}
 ...
 {if $obj->type == 'interface'}
  <a href="{get_url page='Interface Edit' id=$obj->idx}" title="Edit interface {$obj->name}"><img src="{$icon_interfaces}" alt="interface icon" />&nbsp;{$obj->name}</a>{if !isset($smarty.foreach.objects.last) || empty($smarty.foreach.objects.last)},{/if}
 {/if}
{foreachelse}
 none
{/foreach}

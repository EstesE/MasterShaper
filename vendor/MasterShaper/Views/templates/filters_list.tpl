...
{filters_list}
 <tr>
  ...
  <td>
   <div name="filter_{$item->getIdx()}" class="filterable inline editable content" data-current-value="{$item->getName()}" data-orig-value="{$item->getName()}" style="float: left;">{$item->getName()}</div>&nbsp;
   <a name="filter_{$item->getIdx()}" class="inline editable edit link" data-inline-name="filter_{$item->getIdx()}"><i class="tiny edit icon"></i></a>
   <div name="filter_{$item->getIdx()}" class="inline editable formsrc" style="display: none;">
    <form class="ui form" onsubmit="return false;">
     <div class="fields">
      <div class="field small ui input">
       <input type="text" name="filter_{$item->getIdx()}" value="{$item->getName()}" data-action="update" data-model="filter" data-key="filter_name" data-id="{$item->getIdx()}" />
      </div>
      <div class="field">
  ...
  <td>
   <div class="ui icon buttons">
    <a id="edit_link_{$item->getIdx()}" href="{get_url page='filters' mode='edit' id=$item->getSafeLink()}" class="edit item ui icon button action link"><i class="edit icon"></i></a>
    <a id="delete_link_{$item->getIdx()}" class="delete item ui icon button action link" data-action-title="Deleting {$item->getName()|escape}" data-modal-title="Delete {$item->getName()|escape}" data-modal-text="Please confirm to delete {$item->getName()|escape}" data-id="{$item->getIdx()}" data-guid="{$item->getGuid()}" data-model="filter" data-content="Delete {$item->getName()|escape}" data-variation="wide"><i class="remove circle icon"></i></a>

...
class ServiceLevelsView extends DefaultView
{
    ...
    {
        try {
            $this->service_levels = new \MasterShaper\Models\ServiceLevelsModel;
        } catch (\Exception $e) {
            $this->raiseError(__METHOD__ .'(), failed to load ServiceLevelsModel!', false, $e);
            return false;
        }
    ...
    public function showList($pageno = null, $items_limit = null)
    {
        ...
        if (!$this->service_levels->hasItems()) {
        ...
        if (!$pager->setPagingData($this->service_levels->getItems())) {
    ...
    public function service_levelsList($params, $content, &$smarty, &$repeat)
    {

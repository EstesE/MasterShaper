...
{ foreach from=$obj_use_target key=obj_idx item=obj name=objects }
 { if $obj->type == 'group' }
  <a href="{$rewriter->get_page_url('Target Edit', $obj->idx)}" title="Edit target { $obj->name }"><img src="{$icon_targets}" alt="target icon" />&nbsp;{ $obj->name }</a>{ if ! $smarty.foreach.filters.last},{/if}
 { /if }
 { if $obj->type == 'pipe' }

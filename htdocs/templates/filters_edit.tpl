...
<table style="width: 100%;" class="withborder2"> 
 ...
 <tr>
  ...
  <td>
   ...
   <img class="change_to" src="{$icon_arrow_right}" value="Go" onclick="if($('select[name=filter_protocol_id]').val() > 0) change_to('{$rewriter->get_page_url('Protocol Edit', 0)}', $('select[name=filter_protocol_id]').val()); return false ;" title="Click to edit currently selected protocol" />
  </td>
  <td>
 ...
 <tr>
  <td style="text-align: center;"><a href="{$rewriter->get_page_url('Filters List')}" title="Back"><img src="{$icon_arrow_left}" alt="arrow left icon" /></a></td>
...
{if isset($pipe_use_filters) && !empty($pipe_use_filters)}
 ...
 {foreach from=$pipe_use_filters key=pipe_idx item=pipe_name name=pipes}
  <a href="{$rewriter->get_page_url('Pipe Edit', $pipe_idx)}" title="Edit pipe {$pipe_name}"><img src="{$icon_pipes}" alt="pipe icon" />&nbsp;{$pipe_name}</a>{if !isset($smarty.foreach.pipes.last)},{/if}
 {foreachelse}
  none
 {/foreach}
{/if}

...
{if !isset($pipe->pipe_idx) || empty($pipe->pipe_idx)}
...
{if isset($chain_use_pipes) && !empty($chain_use_pipes)}
 ...
 {foreach from=$chain_use_pipes key=chain_idx item=chain_name name=chains}
  <a href="{$rewriter->get_page_url('Chain Edit', $chain_idx)}" title="Edit chain {$chain_name}"><img src="{$icon_chains}" alt="chain icon" />&nbsp;{$chain_name}</a>{if !isset($smarty.foreach.chains.last) || empty($smarty.foreach.chains.last)},{/if}
 {foreachelse}
  none
 {/foreach}
{/if}

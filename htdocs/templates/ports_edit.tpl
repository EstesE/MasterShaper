...
{if isset($filter_use_port) && !empty($filter_use_port)}
 ...
 {foreach from=$filter_use_port key=filter_idx item=filter_name name=filters}
  <a href="{$rewriter->get_page_url('Filter Edit', $filter_idx)}" title="Edit filter {$filter_name}"><img src="{$icon_filters}" alt="filter icon" />&nbsp;{$filter_name}</a>{if !isset($smarty.foreach.filters.last) || empty($smarty.foreach.filters.last)},{/if}
 {foreachelse}
  none
 {/foreach}
{/if}
